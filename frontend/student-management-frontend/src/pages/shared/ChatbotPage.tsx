...
export function ChatbotPage() {
	...
	const sendMessage = async (content: string) => {
		...
		try {
			...
			setMessages((prev) => [
				...prev,
				{ role: "assistant", content: response.reply, componentData: response.componentData },
			]);
	...
	const handleImportSubmit = async (data: {
		...
		try {
			...
			setMessages((prev) => [
				...
				{
					role: "assistant",
					content: `✅ Đã nhận yêu cầu import! <br>📊 Lớp: ${data.classId}<br>👥 Số lượng: ${data.quantity || "Custom"}`,
				},
			]);
			...
			setMessages((prev) => [
				...
				{
					role: "assistant",
					content: "❌ Có lỗi xảy ra khi import. Vui lòng thử lại.",
				},
			]);
		}
	};
				...
				>
					<div className="space-y-3">
						<div className="h-[420px] overflow-y-auto rounded-lg border border-slate-200 bg-slate-50 p-4 space-y-3">
							...
							{activeComponent?.type === "student_import" && (
								<div className="bg-white border border-slate-200 rounded-lg p-4">
									<StudentImportForm
										classId={activeComponent.data.classId}
										departmentId={activeComponent.data.departmentId}
