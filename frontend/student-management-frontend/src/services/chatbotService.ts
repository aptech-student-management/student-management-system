...
const buildFallbackReply = (payload: {
  ...
  if (
    ...
    message.includes("thời khóa biểu") ||
    message.includes("môn học")
  ) {
    return {
      reply:
...
export const askChatbotApi = async (payload: {
  ...
  try {
    ...
    return {
      reply:
        data?.reply?.trim() ||
        "Mình đã nhận câu hỏi, bạn có thể hỏi chi tiết hơn để mình hỗ trợ tốt hơn.",
